 ...
 */
	public function create(&$Model, $data) {
		...
		try{
			$return = $this->_db
				->selectCollection($Model->useTable)
				->insert($data, true);
		} catch (MongoException $e) {
			$this->error = $e->getMessage();
			trigger_error($this->error);
 ...
 * @access public
 */
	public function update(&$Model, $data, $conditions = null) {
 ...
 */
	public function read(&$Model, $query = array()) {
		...
		if (empty($offset) && isset($page) && $limit) {
			$offset = $page * $limit;
		}
		...
		if (empty($modify)) {
			if ($fields == array('count' => true)) {
				...
				return array($Model->alias => array('count' => $count));
			}
