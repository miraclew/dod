...
class Validator
{
    ...
    const NICK_NAME_PATTERN = '/^[^\s]{1,20}$/';
    ...
    public static function isNickNameValid($value) {
    	if (empty($value)) {
    		return false;
    	}
        $pattern = self::NICK_NAME_PATTERN;
        $trimValue = trim($value);
        //设置长度的限制
        if ( strlen(iconv('utf-8', 'gb2312', $value)) > NICKNEMA_LONG )
        	return false;
        if ( strlen(iconv('utf-8', 'gb2312', $value)) < NICKNEMA_SHORT )
        	return false;
        /*if ( !preg_match($pattern, $value) )
        {
            return false;
        }*/
        if (checkFilterWords($value))
        	return false;
        return true;
    }
