 ...
 */
    public function init($settings = array()) {
        parent::init(array_merge(
            array(
                'engine' => 'File', 'path' => CACHE_, 'lock'=> true,
 ...
 */
    public function decrement($key, $offset = 1) {
        throw new HException(__('Files cannot be atomically decremented.'));
    }
 ...
 */
    public function increment($key, $offset = 1) {
        throw new HException(__('Files cannot be atomically incremented.'));
    }
    ...
    protected function _setKey($key, $createKey = false) {
        ...
        if (empty($this->_File) || $this->_File->getBaseName() !== $key) {
            ...
            if (!$exists && !chmod($this->_File->getPathname(), (int)$this->settings['mask'])) {
                trigger_error(__('Could not apply permission mask "%s" on cache file "%s"',
